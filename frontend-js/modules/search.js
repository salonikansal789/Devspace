...
//     // 3.methods
    ...
    sendRequest(){
     ...
     renderResults(posts){
        ...
        const flattenedPostsArray = postsArray.flat();
console.log(flattenedPostsArray.length);
const innerArray = postsArray[0]; // Access the inner array
innerArray.forEach(post => {
  console.log(post.title);
});
  

        // console.log(postsArray.length);
        if(flattenedPostsArray.length){
            this.resultsArea.innerHTML = `<div class="list-group shadow-sm">
            <div class="list-group-item active"><strong>Search Results</strong> (${flattenedPostsArray.length > 1 ? `${flattenedPostsArray.length} items found` : '1 item found'})</div>
            ${innerArray.map(post => {
                let postDate = new Date(post.createdAt)
                return `<a href="/post/${post._id}" class="list-group-item list-group-item-action">
                 <strong>${post.title}</strong>
                <span class="text-muted small"> on  ${postDate.getDate()}/${postDate.getMonth()+1}/${postDate.getFullYear()}</span>
            </a>`
            }).join('')}
            </div>`
        }else{
